...
vignette: |
  ...
  %\VignetteIndexEntry{SPdenovo Template Workflow}
...
Rscript -e "rmarkdown::render('SPdenovo.Rmd', c('BiocStyle::html_document'), clean=TRUE)"
-->
...
Users want to provide here background information about the De novo transcriptome assembly project.
...
while the `SPdenovo.Rmd` script and the `targets` file are expected to be located in the parent directory. The R session is expected to run from this parent directory.
...
Now open the R markdown script `SPdenovo.Rmd`in your R IDE (_e.g._
...
necessary update the corresponding _`.cwl`_ and _`.yml`_ files. A collection of pre-generated _`.cwl`_ and _`.yml`_ files are provided in the _`param/cwl`_ subdirectory of each workflow template. They are also viewable in the GitHub repository of _`systemPipeRdata`_ ([see
